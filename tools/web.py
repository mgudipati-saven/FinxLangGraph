...
# a very basic function that uses BeautifulSoup to parse some HTML content:
...
# a function that uses aiohttp to fetch the content of a webpage:
...
# define our pydantic object with the input arguments for the tool:
class ResearchInput(BaseModel):
  research_urls: list[str] = Field(description="Must be a list of valid URLs.")

# define the tool function that will be called by the API:
...
# if the script is run directly, we can test the tool:
